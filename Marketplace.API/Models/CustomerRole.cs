...
public class CustomerRole : RoleBase
{
    public CustomerRole()
    {
    }

    public override UserRoleKind Kind => UserRoleKind.Customer;

    public override string RoleName => "Customer";
    ...
    public Order PlaceOrder(Guid userId, Dictionary<Product, int> cartItems)
    {
        ...
        var sellerId = cartItems.Keys.First().SellerId;
        if (cartItems.Keys.Any(p => p.SellerId != sellerId))
            throw new InvalidOperationException("Одне замовлення може містити товари лише від одного продавця");

        var order = new Order(userId, sellerId);
